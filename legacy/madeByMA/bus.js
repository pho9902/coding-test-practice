...
const solution = (s) => {
    ...
    return input.map((bus) => {
        return numbers.includes(bus) ? isDriving(bus, today) : `${bus}번 버스는 존재하지 않습니다.`;
    });
};

const isDriving = (bus, today) => {
    const index = timeTable.findIndex(v => v.number === bus);
    const table = (today.day === 0 || today.day === 6) ? timeTable[index].timetable.weekend : timeTable[index].timetable.weekday;
    let result = '';
    ...
    table.forEach(el => {
        ...
        el.arrive < `${today.hour}:${today.minute}` ? null : result += `${bus}번 버스는 ${el.name}를 향해 운행하고 있습니다.`;
    })
